...
function SummaryCard({ label, value, accent }: { label: string; value: number; accent?: boolean }) {
  return (
    <div
      className={cn(
        "rounded-xl border p-4 transition-colors hover:bg-muted/50",
...
export function GuestEditor({ initialGuests, weddingId }: Props) {
  ...
  const groups = useMemo(() => {
    ...
    guests.forEach((g) => {
      if (g.group_name) seen.add(g.group_name)
    })
    return [...seen].sort()
  }, [guests])
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {groups.length > 0 && (
          <Select
            ...
            onValueChange={(v) => setFilterGroup(v === "all" ? "" : v)}
          >
            <SelectTrigger className="w-44">
                ...
                <TableRow key={guest.id}>
                  ...
                  <TableCell>
                    <Badge
                      className={cn(
                        "border text-xs",
                        STATUS_BADGE[guest.rsvp_status] ?? STATUS_BADGE.pending
                      )}
                    >
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button
                        ...
                        size="icon"
                        className="h-9 w-9"
                        aria-label={`Modifier ${guest.first_name ?? ""}`}
                        onClick={() => {
                      ...
                      <Button
                        ...
                        aria-label={`Supprimer ${guest.first_name ?? ""}`}
