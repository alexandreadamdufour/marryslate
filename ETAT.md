...
| **Audit synchro migrations↔prod** | Plusieurs objets créés à la main en prod hors migration (table `rsvp_responses`, policy guestbook, CSP). Vérifier qu'aucun autre écart n'existe. Chantier de fond. Ampleur confirmée : les types régénérés depuis prod révèlent ~47 divergences avec les migrations versionnées. Exemples : `rsvp_responses.guest_count` (`NOT NULL DEFAULT 1` migration → nullable prod), plusieurs colonnes seating/timeline/planner idem, RPC `increment_wedding_view_count` présent en prod mais absent des migrations. `types.ts` commité contient un patch manuel ponctuel (`conflict_resolved_at`) qui devra être retiré au moment de l'audit. Chantier réel : 3-5h. |
