...
- **GA4 + GSC déjà live** (jamais documenté avant ce jour, découvert le 2 juillet en croyant à tort qu'il fallait tout poser) : `NEXT_PUBLIC_GA_ID=G-2R1HR0WJR7` posé sur Vercel Production (confirmé via le HTML rendu de marryslate.com), tag de vérification Google Search Console hardcodé dans `src/app/layout.tsx` (`verification.google`, commit `37b4814`, 24 juin) — vérifié live également. Les deux fonctionnent déjà, ne pas repartir de zéro dessus.
...
## ✅ Résolu — `uploadCoverImage` — 2 juillet 2026

- Gap fermé (bloquant beta) : aucun couple ne pouvait fixer sa photo hero depuis le dashboard. Action `uploadCoverImage` (même pattern que `uploadGiftImage`/`uploadStoryImage`, bucket `gift-images` réutilisé, pas de nouveau bucket) + composant `CoverImageUploader` intégré en première section de `/dashboard/site` (upload + `updateWedding` en un seul geste, pas de bouton "Enregistrer" séparé). Suppression possible. Checklist onboarding "Ajouter votre photo de couple" pointe maintenant vers `/dashboard/site#cover`.
...
| **Overrides `esbuild`/`js-yaml` skippés (arbitrage, 2 juillet)** | Semgrep supply-chain flagge `esbuild@0.21.5` (épinglé par `vite@5.4.21`, transitif) et `js-yaml@3.14.2` (épinglé par `gray-matter@4.0.3`, dépendance production — parsing frontmatter blog, pas dev-only). `pnpm up X@latest` est un no-op sur ces deux (pas de dep directe à bumper). Un vrai fix nécessiterait `pnpm.overrides` — skippé : risque de compat (gray-matter jamais testé avec js-yaml 4.x sur ce projet, esbuild 0.21→0.25+ est un saut de version que vite 5.4.21 ne supporte pas forcément en interne) jugé supérieur au bénéfice pour des transitifs. À retraiter uniquement si un audit reachability Semgrep repasse l'un des deux en critique. |
